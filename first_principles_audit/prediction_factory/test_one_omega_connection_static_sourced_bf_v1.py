...
def test_all_new_identities_and_static_degrees(model):
    ...
    for part in ('material','exterior','reconstruction','norms'):
...
def test_actual_Poisson_current_has_zero_closed_remainder(model):
    ...
    assert gate.plus(r['J_Sigma_actual_port'],gate.spatial_h({15:rho},c))=={}
...
def test_general_closed_correction_and_source_are_both_retained(model):
    ...
    assert jump==r['general_compatible_boundary_current']
...
@pytest.mark.parametrize('mutation',['Einstein','global_static_uniqueness','particular_sign'])
def test_rehashed_mutant_rejected_by_fresh_derivation(payload,mutation):
    ...
    else:bad['model']['reconstruction']['B_part_plus']={}
