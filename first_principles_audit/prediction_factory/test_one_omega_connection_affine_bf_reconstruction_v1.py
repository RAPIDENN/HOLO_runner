...
def test_basis_counts_and_all_operator_identities(model):
    ...
    for part in ('complex','lift','affine','norms'):
...
def test_explicit_current_lift_with_cutoff_derivative_has_the_required_signs():
    ...
    assert gate.plus(gate.trace(B,c),gate.scale(-1,gate.trace(gate.scale(-1,B),c)))==J
...
def test_controls_and_remaining_limits(model):
    assert len(model['negative_controls'])==8 and all(model['negative_controls'].values())
...
@pytest.mark.parametrize('mutation',['erase_theta','source_order_two','flip_B_minus'])
def test_rehashed_receipt_mutations_require_fresh_derivation(payload,mutation):
    ...
    else:bad['model']['lift']['B_minus']=deepcopy(bad['model']['lift']['B_plus'])
