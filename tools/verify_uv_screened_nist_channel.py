...
if __name__ == "__main__":
    ...
    for label, obj in [("naive", naive), ("uv", uv)]:
        ...
        # The public bundle may redact observed NIST series (y_obs) to avoid re-packaging
        # third-party time series. In that case, metrics are retained but cannot be recomputed.
        if "y_obs" in obj["series"]:
            ...
            if abs(chi2 - chi2_reported) > 1e-6 * max(abs(chi2_reported), 1.0):
                raise SystemExit(
                    f"[ERR] {label} chi2 mismatch vs reported (recomputed={chi2}, reported={chi2_reported})"
                )
            ...
            if abs(r - r_reported) > 1e-6 * max(abs(r_reported), 1.0):
                raise SystemExit(
                    f"[ERR] {label} pearson_r mismatch vs reported (recomputed={r}, reported={r_reported})"
                )
        else:
            if "y_pred" not in obj["series"]:
                raise SystemExit(f"[ERR] {label} comparison missing series.y_pred")
            if "redaction" not in obj:
                raise SystemExit(f"[ERR] {label} missing series.y_obs but no redaction metadata present")
            y_pred = list(map(float, obj["series"]["y_pred"]))
            n = int(obj["metrics"]["n"])
            if n != len(y_pred):
                raise SystemExit(f"[ERR] {label} metrics.n does not match series.y_pred length")
            r_reported = float(obj["metrics"]["pearson_r"])
            if not (math.isfinite(r_reported) and -1.0 <= r_reported <= 1.0):
                raise SystemExit(f"[ERR] {label} metrics.pearson_r is not finite / in [-1,1]")
            chi2_reported = float(obj["metrics"]["chi2"])
            chi2n_reported = float(obj["metrics"]["chi2_over_n"])
            if not (math.isfinite(chi2_reported) and math.isfinite(chi2n_reported)):
                raise SystemExit(f"[ERR] {label} metrics.chi2 / chi2_over_n not finite")
